...
- `src/i18n/` is the source of truth for the two supported UI locales and their
  `common`, `docs`, `navigation`, `tools`, and `errors` namespaces.
...
Supporting explanatory documents that are also paired are public/fonts/MANIFEST.md,
the two SSRF harness READMEs, docs/docker-development.md,
docs/architecture/language-switcher.md, and docs/operations/production-hardening.md,
each with its Traditional Chinese companion. docs/mermaid-converter.md,
docs/quality-baselines.md, and the dated records under docs/research/ are
developer-facing notes kept in English only, as are the AI agent instructions in
`AGENTS.md` and `.agents/`. The engineering
...
├── playwright.config.js      Playwright browser-journey runner configuration
├── knip.json                 Knip dependency and dead-code entry points
├── wrangler.jsonc            Cloudflare Pages topology and service bindings
├── .nvmrc                    Node 22 default for local development
├── .dev.vars.example         Local Pages Functions variable template
...
│   ├── architecture/         Paired language-switcher design note
│   ├── operations/           Paired production-hardening and HSTS runbook
│   ├── research/             Dated one-off investigation records
│   ├── mermaid-converter.md  Mermaid route processing, security, and export model
│   ├── quality-baselines.md  Coverage tiers, lint budget, and Axe exceptions
...
├── workers/
│   └── rate-limiter/         Service-bound rate-limiter Worker and its wrangler config
├── test/
│   └── integration/          SSRF harness and other integration fixtures
└── e2e/                      Playwright browser journeys and accessibility specs
```
...
the five paired namespaces under `src/i18n/locales/en-US/` and
`src/i18n/locales/zh-TW/`: `common`, `docs`, `navigation`, `tools`, and `errors`.
The `mermaid` resource file is merged into the `tools` namespace at initialization.
...
| `mermaid` | Browser-local diagram rendering for the Mermaid Converter, imported only after that route requests a render. |
